> ⚠️ DRAFT — not yet publishable. Blockers mirror the privacy policy:
> (1) legal-entity decision (sets the provider/governing-law details below);
> (2) final billing terms once Apple/Google IAP is configured; (3) paid legal
> review (~€150, ideally same review as PRIVACY_POLICY.md). Open items are
> marked **[TODO]**. See legal.md for the handoff.

# Terms of Service — MindFlow

> **App name:** MindFlow (the name shown in the App Store and Google Play).
> The in-app AI companion is a feature of MindFlow — not a separate product
> or company.

**Last updated:** 2026-06-20
**Effective date:** [PLACEHOLDER — set to publication date]
**Version:** 1.0 (pre-release draft)

---

## 1. Who We Are and What These Terms Cover

These Terms of Service ("Terms") are a binding agreement between you and the
provider of MindFlow ("MindFlow", "we", "us") governing your use of the
MindFlow mobile application and related backend services (together, the
"Service").

> **[TODO — ENTITY DECISION PENDING]** The provider is either Alexandra
> Tomulescu (sole trader / autónomo) **or** a Spanish Sociedad Limitada (SL)
> to be incorporated. Finalise the provider name, address, and registration
> details once the entity is chosen. This also determines the governing-law
> clause in Section 15. See legal.md.

**Provider:** **[Alexandra Tomulescu — sole trader] / [MindFlow SL — TBC]**
Valencia, Spain (EU)
Contact: support@mindflowjournal.app

By creating an account, ticking the acceptance box, or using the Service, you
confirm that you have read, understood, and agree to these Terms and to our
[Privacy Policy](https://mindflowjournal.app/privacy). If you do not agree, do
not use the Service.

---

## 2. Eligibility

You must be at least **16 years old** to use MindFlow. By using the Service you
confirm that you meet this requirement. The Service is not directed at children
(see the Privacy Policy, Section 9).

---

## 3. The Service

MindFlow is a personal journaling application. It lets you write journal
entries, record mood scores, view trends and insights, and—optionally—interact
with **MindFlow's AI companion**, which reflects on your entries.

We may add, change, or remove features over time. We may also impose or change
usage limits between the free and premium tiers (see Section 8).

---

## 4. AI Companion — Important Disclaimer

**MindFlow's AI companion is not a therapist, psychologist, doctor, or any kind
of medical or mental-health professional. It does not provide medical advice,
diagnosis, treatment, or crisis intervention.** Its responses are AI-generated,
may be inaccurate or inappropriate, and must not be relied upon as professional
advice.

The Service is for personal reflection and journaling only. It is **not a
medical device** and is not a substitute for professional care.

**If you are in distress or experiencing a mental-health crisis, contact a
qualified professional or your local emergency services.** The app links to
findahelpline.com, an international directory of crisis-support resources.

Use of the AI companion requires your separate explicit consent to AI processing
of your content, as described in the Privacy Policy (Section 5). You can disable
the AI companion at any time in Settings → Privacy.

---

## 5. Your Account

You are responsible for:
- providing accurate registration information;
- keeping your login credentials (and any biometric unlock you enable) secure;
- all activity that occurs under your account.

Notify us promptly at support@mindflowjournal.app if you suspect unauthorised
use of your account.

---

## 6. Acceptable Use

You agree not to:
- use the Service for any unlawful purpose or in violation of these Terms;
- attempt to gain unauthorised access to the Service, other accounts, or our
  systems;
- interfere with, disrupt, overload, or probe the Service or its
  infrastructure;
- reverse engineer, decompile, or attempt to extract source code, except where
  such restriction is prohibited by law;
- use the Service to generate, store, or transmit content that is unlawful or
  infringes the rights of others;
- attempt to manipulate, abuse, or circumvent usage limits, the AI companion,
  or rate limits.

We may suspend or terminate accounts that violate this section (see Section 11).

---

## 7. Your Content

**You own your content.** Journal entries, mood scores, and messages you create
("Your Content") remain yours. We do not claim ownership of Your Content.

You grant us a limited, non-exclusive licence to store, process, and display
Your Content **solely to operate and provide the Service to you** (for example,
to save your entries, generate insights, and—where you have consented—produce
the AI companion's responses). This licence ends when Your Content is deleted, except for
backups in the ordinary course and anonymised records as described in the
Privacy Policy.

**We will never sell Your Content or use it for advertising.** How we handle
Your Content, including special-category (mental-health) data and the consents
we rely on, is described in the [Privacy Policy](https://mindflowjournal.app/privacy).

You are responsible for Your Content and confirm you have the right to provide
it.

---

## 8. Subscriptions, Billing, and Free Tier

MindFlow offers a free tier with usage limits and a paid premium subscription.

**Billing through the app stores.** Premium subscriptions are sold and processed
exclusively through **Apple In-App Purchase** (iOS) and **Google Play Billing**
(Android). We do not receive or store your payment card details. Your purchase
is also subject to the terms of the relevant app store.

**Auto-renewal.** Subscriptions renew automatically for successive periods
unless cancelled at least 24 hours before the end of the current period.
**You manage and cancel your subscription through your Apple App Store or Google
Play account settings**, not within MindFlow.

**Refunds.** Refunds are handled by Apple or Google under their respective
policies. We generally cannot issue refunds directly for store purchases.

**Price and plan changes.** We may change subscription prices or plan limits.
Changes to recurring prices will be notified in advance and apply from your next
billing period; you may cancel before they take effect.

> **[TODO — BILLING]** Finalise once Apple/Google IAP is configured: exact
> renewal periods and prices, free-tier limits (entries / AI messages),
> trial terms (if any), and EU-specific consumer withdrawal-right wording for
> digital subscriptions. Confirm whether your own EULA or Apple's standard
> Licensed Application EULA applies (Apple requires one or the other). See
> legal.md / COMPLIANCE.md.

---

## 9. Intellectual Property

The Service itself—including the MindFlow name, the app, its design,
and underlying software—is owned by us or our licensors and is protected by
intellectual-property laws. These Terms do not grant you any rights in our
intellectual property except the limited right to use the Service as permitted
here. This does not affect your ownership of Your Content (Section 7).

---

## 10. Privacy

Our handling of personal data is governed by the
[Privacy Policy](https://mindflowjournal.app/privacy), which forms part of your
agreement with us. Please read it carefully, particularly the sections on
special-category data and AI processing.

---

## 11. Suspension and Termination

**By you.** You may stop using the Service at any time and delete your account
in Settings → Delete Account. Deletion permanently removes your personal data
as described in the Privacy Policy (Section 4).

**By us.** We may suspend or terminate your access if you materially breach
these Terms, use the Service unlawfully, or where required by law. Where
reasonable and lawful, we will give notice.

**Inactivity.** Accounts inactive for 12 months are deleted automatically after
prior email warnings, as described in the Privacy Policy (Section 4).

Sections that by their nature should survive termination (including Sections 7,
9, 12, 13, and 15) continue to apply.

---

## 12. Disclaimers

To the fullest extent permitted by law, the Service is provided **"as is" and
"as available"** without warranties of any kind, whether express or implied,
including fitness for a particular purpose and non-infringement. We do not
warrant that the Service will be uninterrupted, error-free, or that AI-generated
content will be accurate or suitable.

Nothing in these Terms excludes or limits rights that cannot be excluded or
limited under applicable mandatory consumer-protection law.

---

## 13. Limitation of Liability

To the fullest extent permitted by law, we will not be liable for any indirect,
incidental, special, consequential, or punitive damages, or for any loss of
data, profits, or goodwill, arising from or related to your use of (or inability
to use) the Service.

Our total aggregate liability for any claim arising out of or relating to the
Service is limited to the greater of (a) the amount you paid us for the Service
in the 12 months before the claim, or (b) €50.

**These limitations do not apply** to liability for death or personal injury
caused by negligence, for fraud, or to any other liability that cannot be
limited or excluded under applicable law (including your mandatory rights as a
consumer in the EU).

---

## 14. Changes to These Terms

We may update these Terms from time to time. For material changes, we will give
notice by email and/or in-app at least 30 days before they take effect.
Continued use of the Service after the effective date constitutes acceptance of
the updated Terms. The current version is always available within the app and at
https://mindflowjournal.app/terms.

> **[TODO]** If you introduce Terms versioning and re-acceptance, record the
> accepted version per user (a `terms_version` field is not yet implemented —
> currently only `terms_accepted_at` is stored). See legal.md.

---

## 15. Governing Law and Disputes

> **[TODO — DEPENDS ON ENTITY]** Confirm governing law and venue once the
> entity is chosen. Default expectation: laws of **Spain**, with disputes
> before the competent courts of the provider's seat. Note that as an EU
> consumer you may also benefit from the mandatory laws and courts of your own
> country of residence — this clause must not deprive you of those rights.
> Consider referencing the EU Online Dispute Resolution platform.

These Terms are governed by the laws of **Spain**, without prejudice to the
mandatory consumer-protection rights you have in your country of residence in
the EU.

---

## 16. Contact

Questions about these Terms:

**[Provider — see Section 1 TODO]**
Email: support@mindflowjournal.app

> **[TODO]** Set up and monitor support@mindflowjournal.app (and the
> privacy@ address referenced in the Privacy Policy) before publishing.

---

*MindFlow — your private journaling companion.*
