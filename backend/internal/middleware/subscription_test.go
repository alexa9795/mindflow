...
func TestRequireAISubscriptionMiddleware(t *testing.T) {
	tests := []struct {
		name           string
		userID         string
		status         *subscription.SubscriptionStatus
		...
		{
			name:           "CanUseAI true passes through to next handler",
			userID:         "user-1",
			status:         &subscription.SubscriptionStatus{CanUseAI: true},
			wantStatus:     http.StatusOK,
			wantNextCalled: true,
		},
		{
			name:        "CanUseAI false returns 403 with AI_SUBSCRIPTION_REQUIRED",
			userID:      "user-1",
			status:      &subscription.SubscriptionStatus{CanUseAI: false},
			wantStatus:  http.StatusForbidden,
			wantErrCode: "AI_SUBSCRIPTION_REQUIRED",
		},
		{
			name:       "service error returns 500",
			userID:     "user-1",
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			svc := &mockSubSvc{status: tc.status, err: tc.svcErr}
			nextCalled := false
			handler := middleware.RequireAISubscription(svc)(makeNext(&nextCalled))
